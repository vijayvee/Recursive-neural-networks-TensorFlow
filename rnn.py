...
class RNN_Model():
    ...
    def add_model_vars(self):
        ...
        with tf.variable_scope('Composition'):
            tf.get_variable("L",shape=[self.vocab.total_words,self.config.embed_size])
    ...
    def add_model(self, node):
        ...
        if node.isLeaf:
            wordIndex=self.vocab.encode(node.word)
            curr_node_tensor=tf.expand_dims(tf.gather(L,wordIndex),0)
        else:
